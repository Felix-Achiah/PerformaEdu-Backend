...
from .models import Class, Subject, TeacherLevelClass, Student, ClassEnrollment, HistoricalClassEnrollment, Assessment, StudentParentRelation, SubjectPerformance, ProcessedMarks, TimeTable
from user_auth.models import User
from .serializers import ClassSerializer, SubjectSerializer, TeacherLevelClassSerializer, StudentSerializer, AssessmentSerializer, PromoteStudentsSerializer, ClassEnrollmentSerializer, SubjectPerformanceSerializer, TopicPerformanceSerializer, ProcessedMarksSerializer, StudentParentRelationSerializer, TimeTableSerializer
...
def register_teacher_subjects(request):
    if request.method == 'POST':
        ...
        existing_subjects = TeacherLevelClass.objects.filter(
            teacher=request.user, class_id=request.data['class_id']
        )
...
def update_teacher_subjects(request, class_id):
    if request.method == 'PUT':
        ...
        try:
            teacher_level_class = TeacherLevelClass.objects.get(
                teacher=request.user, class_id=class_id
            )
        except TeacherLevelClass.DoesNotExist:
            return Response(
                {'error': 'TeacherLevelClass not found.'},
                status=status.HTTP_404_NOT_FOUND
            )

        existing_subject_ids = set(teacher_level_class.subjects_taught.values_list('id', flat=True))
        new_subject_ids = set(request.data.get('subjects_taught', []))

        # Handle the addition and removal of subjects
        subjects_to_add = new_subject_ids - existing_subject_ids
        subjects_to_remove = existing_subject_ids - new_subject_ids

        # Update subjects in the TeacherLevelClass object
        for subject_id in subjects_to_remove:
            teacher_level_class.subjects_taught.remove(subject_id)
        for subject_id in subjects_to_add:
            teacher_level_class.subjects_taught.add(subject_id)

        # Serialize and return the updated object
        serializer = TeacherLevelClassSerializer(teacher_level_class, context={'request': request})
...
# Register Students to a class
@api_view(['POST'])
@permission_classes([permissions.IsAuthenticated, IsTeacher])
def create_students(request):
    if request.method == 'POST':
        student_data = request.data.get('students', [])  # Get the list of student data from the request
        ...
        for data in student_data:
            ...
            existing_student = Student.objects.filter(name=student_name).exists()
            if existing_student:
                errors.append({'error': f'A student with the name "{student_name}" already exists.'})
            else:
                class_enrollment_data = data.get('class_enrolled')
                if not class_enrollment_data:
                    errors.append({'error': 'Class enrollment data is required.'})
                elif 'class_id' not in class_enrollment_data:
                    errors.append({'error': 'Class ID is required in class enrollment data.'})
                else:
                    class_id = class_enrollment_data['class_id']
                    ...
                    try:
                        with transaction.atomic():
                            ...
                            student = Student.objects.create(
                                name=student_name,  date_of_birth=data.get('date_of_birth'),
                                gender=data.get('gender')
                            )

                            # Create ClassEnrollment instance
                            class_enrollment = ClassEnrollment.objects.create(student_id=student.id, class_id=class_instance, academic_year=academic_year)
        ...
        if errors:
            ...
            return Response(created_students, status=status.HTTP_201_CREATED)
...
def get_students_by_class_id(request):
    ...
    try:
        # Filter ClassEnrollment by class_id and student status 'existing'
        enrollments = ClassEnrollment.objects.filter(class_id=class_id, status='existing')
...
def update_student(request, student_id):
    ...
    if student_serializer.is_valid():
        ...
        if 'class_enrollment' in request.data:
            class_enrollment_data = request.data['class_enrollment']
            print(class_enrollment_data)
            for enrollment in class_enrollment_data:
                enrollment_id = enrollment.get('id')
                if enrollment_id:
                    class_enrollment = ClassEnrollment.objects.get(id=enrollment_id)
                    class_enrollment_serializer = ClassEnrollmentSerializer(class_enrollment, data=enrollment)
                    if class_enrollment_serializer.is_valid():
                        class_enrollment_serializer.save()
                else:
                    new_enrollment = ClassEnrollment(student=student, **enrollment)
                    new_enrollment.save()
